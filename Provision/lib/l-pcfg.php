...
Date        Who     What
----        ---     ----
 2-Nov-04   EWB     factored out of patch/wu-config.php
 3-Nov-04   EWB     uses l-rlib find_glbl_pcfg.
10-Nov-04   EWB     PatchConfig.configtype
11-Nov-04   EWB     Scope for all my machines.
28-Dec-04   BTE     Defaults for PatchConfig.chain, PatchConfig.chainseconds,
                    and WUConfig.restart.
29-Dec-04   BTE     Moved PatchConfig.chain, .chainseconds to WUConfig.chain,
                    .chainseconds.
23-Feb-05   EWB     find_mgrp_host works, even on renamed machines.
12-Sep-05   BTE     Added checksum invalidation code.
23-Sep-05   BJS     Added okcancel_link().
07-Dec-05   BJS     find_host_mgrp -> PCFG_find_host_mgrp, find_site_mgrp ->
                    PCFG_find_site_mgrp.
13-Dec-05   BJS     Added back button, removed PCFG from
                    find_host_mgrp/site_mgrp.
14-Dec-05   BJS     error_log no longer includes PCFG_
26-Jan-06   BTE     Bug 3059: Redesign DSYN and tables to "remotable" internal
                    pointers.
30-Jan-06   BJS     Added constButtonReset.
26-May-06   BTE     Bug 3293: Allow name changes for groups.
19-Jul-06   BTE     Bug 3239: 913 errors on CI server.
20-Sep-06   BTE     Some cleanup to work with l-tiny.php.  Other changes to
                    support PatchConfig.wpgroupid.
...
*/
   ...
   /*
    |  This is a large hunk of utility code which was factored out of
    |  wu-confg.php.  It could have just stayed as there, but at over
    |  10,000 lines, wu-confg was becoming unweildy.
    */
 ...
 // }
   ...
   /*
    |  We'll also save away the site and host
    |  so that scop_info can use it later.
    */
        ...
        if (($cen) && (!$row))
        {
            ...
            if ($grp)
            {
                ...
                if(!($test))
                {
                    ...
                    {
                        logs::log(__FILE__, __LINE__, "find_host_mgrp: PHP_DSYN_InvalidateRow"
                            . "returned " . $err, 0);
                    }
                }
                if($err==constAppNoErr)
   ...
   /*
    |  This depends on some extra information
    |  that find_host_mgrp saves for us.
    |
   ...
   /*
    |  If the user wants him to schedule
    |  something for February 31, we'll
    |  just go ahead and let him do it.
    |
    |  The client has to handle illegal
    |  dates anyway so just go ahead.
    */
   ...
   /*
    |  day_options   --> WUConfig.installday
    |  wday_options  --> PatchConfig.schedweek
    |  mday_options  --> PatchConfig.schedday
    |  month_options --> PatchConfig.schedmonth
    */
    ...
    // PatchConfig.schedweek
    // PatchConfig.notifydweek
   ...
   /*
    |  The method wizard refers to decline as 'manual'
    |  and approve as 'automatic'.
    */
   ...
   /*
    |  Update an existing pcfg record, and also
    |  takes care of lastupdate.  However, note
    |  that ($pid,$gid,$jid) are invariant.
    */
   ...
   /*
    |  Update an existing wcfg record, and also
    |  takes care of last update.  However, note
    |  that ($wid,$gid) are invariant.
    */
    ...
    /* PCFG_AuditChange
        ...
        Examines the differences between $pcfg and $getsql and audits the
        result.
    */
    function PCFG_AuditChange($env, $getsql, $pcfg, $db)
    {
        ...
        if($set)
        {
            foreach ($set as $key => $row)
            {
                ...
                if($changes)
                {
                    ...
                    {
                        /* This is a type group association, so link the name
                            with the original wizard. */
                        $sql = "SELECT name FROM PatchGroups where pgroupid="
                            . $row['wpgroupid'];
                        $row2 = find_one($sql, $db);
                        if($row2)
                    ...
                    /* Add this information to the audit details */
                    if($details)
                    {
                        $details .= "<br><br>";
                    }
        ...
        if($details)
        {
            ...
            if($err!=constAppNoErr)
            {
                logs::log(__FILE__, __LINE__, "PHP_AUDT_LogLocalAudit failed for MUM: $err", 0);
            }
        }
    }
