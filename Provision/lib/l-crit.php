...
Date        Who     What
----        ---     ----
 8-Apr-03   NL      Make arrow gif path smart, based on $comp['odir']
15-Apr-03   NL      Use get_argument() to set $editmode - fixes bud where not displaying existing rows.
*/


// NINA: need to reorder blocks to take care of empty blocks
// remember to add ASsetSearchCriteria id to table


/*---------------------------------------------------------------------------\
| This PHP function writes the Javascript functions                          |
| that allow rows of the Criteria Builder to be turned                       |
| on and off using Dynamic HTML (= Javascript plus Cascading                 |
| Style Sheets plus the browser's Document Object Model to                   |
| manipulate the location and style of browser elements).                    |
| Unfortunately, the DHTML is ugly and complex because:                      |
|   1) We need to use tables to display the Criteria Builder,                |
|      and <TABLE>, <TR>, and <TD> tags dont inherit styles                  |   
|      reliably, so each one has to have the style repeated                  |
|      (see the HTML below).                                                 |
|   2) That means we need to change the display style for each               |   
|      TD within the TR we want to turn on or off.                           |
|   3) It is easier, and has faster processing, to get and change            |
|      the style of an element by id than by class.*                         |
|      Therefore, we have to change the display style property for each      |
|      individual element based on its id, rather than assigning             |
|      a common class to all the elements of interest                        |
|      and simply changing the display style for that class.                 |
|                                                                            |
| Note also that THIS SCRIPT CALLS JAVASCRIPT COOKIE FUNCTIONS IN FTIENS.JS  |
|                                                                            |   
| - Nina Lauderdale, 8/22/02                                                 |
|                                                                            |
| * (See <http://developer.apple.com/internet/javascript/dynamicforms.html>  |
|   (rental form) and                                                        |
|   <http://developer.apple.com/internet/javascript/rental_form.html>        |
|   versus <http://www.markup.co.nz/dom/styleClass_with_dom.htm>)            | 
\-------------------------------------------------------------------------- */      
    
function outputJavascriptCritBldr($num_ANDs, $num_ORs, $openblockrows) {  # max allowed is (9,9)
    global $QUERY_STRING; 
    global $comparison_options;  
    
    # Are we editing or duplicating?
...
<?php     
    ...
    # In edit mode, open all the rows that contain data
    if ($editmode) 
    {
?>    
