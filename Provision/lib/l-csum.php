...
Date        Who     What
----        ---     ----
21-Apr-04   EWB     Created.
 1-Jun-05   EWB     update for gconfig database
12-Oct-05   BTE     Changed references from gconfig to ".$GLOBALS['PREFIX']."core.
24-Jun-06   BTE     Added dirty_group.
...
/*
    |  We're about to change a local variable,
    |  mark the checksum as invalid (dirty)
    */
...
/*
    |  We're about to change a global variable,
    |  mark the site checksum as invalid (dirty)
    */
...
/*
    |  We're just changed a config setting
    |  for the machine, and we're not sure
    |  exactly what might have been affected.
    |
    |  In this case it's best to be conservative
    |  so we'll just set the dirty bit for both
    |  the host and the site.
    */
...
/* dirty_group
        ...
        Invalidates LegacyCache for members of the group $gid.
    */
function dirty_group($gid, $db)
{
    ...
    if ($row) {
        if (strcmp($row['category'], 'Site') == 0) {
            /* Invalidate the global checksum too */
