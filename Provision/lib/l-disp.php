...
/*  
  HELPER FUNCTIONS USED FOR LEFT PANE 
*/

/*----------------------------------------------------------\
    |  flatten_array takes a 2D array and flattens it into      |
    |  an array w/ no reference to the original keys.           |
    |  Output is an array.                                      |
    |                                                           |
    |  params:                                                  |
    |  $main_array: the 2D associative array                    |
    |  $index_array: an array of elements which are also the    |
    |       of the main array                                   |
...
// stolen right off the dechex PHPman page
/*
...
function get_statuscfg($userid, $db)
{
    ...
    // It may be possible to generate a status == 0 (e.g. a machine group w/ no machines)
...
/*  
  HELPER FUNCTIONS USED FOR RIGHT PANE 
*/


/*
    Functions used for Selection Tables
    
    First are 2 configuration functions, return_tableids() & return_seltbl_args()
    followed by several functions created by Brian that call his C code.
   */
...
function return_tableids($selected)
{
    ...
    // list of names of dbtables for which selection tables should be rendered
    switch ($itemtype) {
...
/*
    return_seltbl_args() is a configuration array for use w/ 
    Brian's Selection Table functions.
    NOTE: Once the new XXXDisplay tables are ion place, make sure
        the database field names below are correct.
   */
...
function gen_seltables($env, $selected, $db)
{
    ...
    foreach ($tableids as $tablename => $tableid) {
        ...
        foreach ($seltbl_args['columns'] as $colname => $args) {
            /* For each column, build the default settings into display.ColumnOptions table*/
        ...
        /* Make sure the users have at least the default settings. */
...
/*
    Functions used for generating the right pane HTML Selection Tables
   */
function gen_heading($itemtype, $itemid, $db)
{
    $heading = "";

    // first lets make sure an item is selected
    if (!$itemtype) {
...
/*----------------------------------------------------------\
    |  create_snapshot_link() creates the 3 html links for      |
    |  entering the snapshot from the status display.           |
    |                                                           |
    |  This link only appears when a machine group or machine   |
    |  is selected in the left pane.                            |
    |                                                           |
    |  $parent is a kludge so that we can determine the         |
    |  mgroupid & dispid from dashboard.Expansions.             |
    |  Because a machine can be in multiple machinegroups and   |
    |  a machinegroup can be in multiple displays, it's tricky  |
    |  to figure out which machinegroup & display               |
    |  a specified machine is in (can't use the arrays).        |
    |  The Expansions table is the only thing that keeps track  |
...
function create_snapshot_link($itemtype, $itemid, $parent, $db)
{
    ...
    if ($itemtype == constDisplayItemMachineGroup) {
        ...
        $act        = 301;
        $censusid   = $itemid;
        // for machine --> need display and machinegroup
...
/*----------------------------------------------------------\
    |  create_restart_link() creates the html links for         |
    |  restart monitoring.                                      |
    |                                                           |
    |  This link only appears when a display is selected        |
    |  in the left pane.                                        |
    |                                                           |
    |  params:                                                  |
    |  $itemtype: the type of item                              |
