...
function DeleteAsset()
{
    ...
    if (id != '')
    {
        func = "function=clear_machine&machine=<?php if (isset($machinename)) echo urlencode($machinename); ?>&site=<?php if (isset($sitename)) echo urlencode($sitename); ?>"
    ...
    {
        func = "function=deleteasset&site=<?php if (isset($sitename)) echo urlencode($sitename); ?>"
            + '&csrfMagicToken=' + csrfMagicToken;
...
function assetfilterClick(){
  ...
  if(id != ''){
  $.ajax({
      url : 'changestatus.php?action=add&qid='+id+'&auth='+auth,
      type : "post",
