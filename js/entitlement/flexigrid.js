...
(function($){
	...
	$.addFlex = function(t,p)
	{
		...
		p = $.extend({
			 ...
			 minheight: 455, //min height of columns
			 ...
			 rpOptions: [10,15,18,20,22,25,40],
			 title: false,
			...
			populate: function () { //get latest data
				...
				if (p.params)
					...
					$.ajax({
					   type: p.method,
					   url: p.url,
					   data: param+"&csrfMagicToken=" + csrfMagicToken,
					   dataType: p.dataType,
					...
					(
						function ()
							...
							.click(
								function (e) 
									{ 
										var selId = document.getElementById('selected').value;
										var thisId = this.id.substring(3);
										var items = $('.trSelected',g.bDiv);
										for(var i=0; i<items.length; i++)
										{											
											$("#"+items[i].id).removeClass('trSelected');
										...
										if(selId != thisId)
										{
											...
											document.getElementById('selected').value = thisId;
										}
										else
										{
											document.getElementById('selected').value = '';
		...
		if (p.width!='auto' && p.width.toString().indexOf('%') < 0)
		...
		//set hTable
			...
			$(g.hDiv).append('<div class="mhDiv table"></div>');
