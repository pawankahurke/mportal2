...
<script lang="javascript">
    ...
    $(document).ready(function() {
        ...
        $("#jsonAlert").hide();
        var dartno = "<?php echo url::toText(url::requestToText('dartno')); ?>";
        ...
        $('#collapseForm').css('height', '100%').addClass('show');
        ...
        if (validateform()) {
            ...
            if (exeValdata == "" && (isEmptyJSONObj(finalSubValues) || JSON.stringify(finalSubValues).length <= 2)) {
                ...
                $.ajax({
                    ...
                    dataType: "json",
                    data: {
                        'function': 'submit_JsonSchema',
                        'dartno': dartno,
                        'precedence': precedence,
                        'data': JSON.stringify(finalSubValues),
                        'csrfMagicToken': csrfMagicToken
                    },
                    success: function(jsonobj) {
                        $('#loader_submit').show();
                        rightContainerSlideClose('configuresideBar');

                        if (jsonobj.msg == 'Success' || jsonobj.msg == '"Success"') {
                            $.notify("Dart-" + dartno + " configured successfully");
                            // debugger
                            // location.reload();
                        } else {
                            console.error("Dart-" + dartno + " configured error", jsonobj)
                            if (jsonobj.msg) {
                                $.notify(jsonobj.msg);
                            } else {

                                var errormess = (jsonobj.replace(/(\r\n|\n|\r)/gm, " "));
                                $.notify("Dart-" + dartno + "<br/>Something went wrong. Try again" + "<br/>" + errormess);
                            }
                        }
                    },
                    error: function(jsonobj) {
                        $.notify("Dart-" + dartno + " configured error");
                        console.error("Dart-" + dartno + " configured error", jsonobj)
                    }
                });
            }
