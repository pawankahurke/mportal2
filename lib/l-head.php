...
function redirect_restricted_login($name)
{
    ...
    $http = ($ssl) ? 'https' : 'http';
    $odir = $comp['odir'];
    $page = ($odir) ? "$odir/acct/files.php" : 'Dashboard/acct/files.php';
...
function standard_style()
{
    ...
    <style type="text/css">
        BODY, TD, TH, P  {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                          font-size: 10pt;}
        .blue         {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       color:333399;}
        .red          {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       color:FF0000;}
        .heading      {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       font-size: $font_heading;
                       color:333399;}
        .subheading   {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       color:333399; font-weight:bold;}
        .footnote     {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       font-size: $font_footnote;}
        .faded        {font-family: Calibri, Verdana, Arial, Helvetica, sans-serif;
                       color: #666666}
...
function check_cache($nocache, $user, $db)
{
    $disable = intval(SERV_GetLeveledOption(
        'disable_cache',
        $user,
        0,
        $db
...
function extended_html_header(
    ...
    check_cache($nocache, $authuser, $db);
...
function head_standard_html_footer($user, $db)
{
    ...
    $copy_rights = '&copy; 2000-2008 HandsFree Networks<br>';
    $footer_left = '159 Kinsley Street<br>';
    $footer_left .= 'Nashua, NH 03060-3763';

    $copy_rights = server_def('copy_rights', $copy_rights, $db);
    $footer_left = server_def('footer_left', $footer_left, $db);

    $footer_left = get_user_options('footer_left', $user, $footer_left, $db);

    $copy_rights = str_replace("\r\n", "\n", $copy_rights);
    $footer_left = str_replace("\r\n", "\n", $footer_left);
    $footer_right = str_replace("\r\n", "\n", $footer_right);

    $right = fontspeak($footer_right);
    $left = fontspeak($footer_left);
    $msg = <<< HERE
