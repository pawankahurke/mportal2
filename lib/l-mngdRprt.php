...
require_once '../libraries/Spout/Autoloader/autoload.php';
...
function getAssetQueries($viaAjax)
{
    ...
    $sql = "SELECT id,name,displayfields FROM " . $GLOBALS['PREFIX'] . "asset.AssetSearches order by name asc";
...
function getEventFilters($viaAjax)
{
    ...
    $sql = "SELECT id,name,eventtag FROM " . $GLOBALS['PREFIX'] . "event.SavedSearches where eventtag !='' order by name asc";
...
function getPatchDetails($viaAjax)
{
    ...
    global $global;
    if ($viaAjax) {
        $year = url::issetInRequest('year') ? url::requestToAny('year') : '2016';
        $mnth = url::issetInRequest('mnth') ? url::requestToAny('mnth') : '0';
    } else {
        $mnth = $global['mum'][0];
        $year = $global['mum'][1];
    }
...
function fetch_users($global, $superGlobal, $infPortal, $owner, $db)
{
    ...
    if ($superGlobal && $infPortal) {
        ...
        if ($global) {
            $users = fetch_all_users_siblings($owner, $db);
        } else {
            $users = array($owner);
        }
    }
...
{
    $sql                        = "SELECT name,global,include,username,created,infportal, " .
        "emaillist,userid,type FROM " . $GLOBALS['PREFIX'] . "report.ManagedReport WHERE id =$id";
    ...
    $sqlSec                     = "SELECT sectionid FROM " . $GLOBALS['PREFIX'] . "report.ManagedReportMap WHERE reportid = $id";
