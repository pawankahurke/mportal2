...
function createCoreUser($pdata)
{
    ...
    if ($res['usercnt'] > 0) {
        ...
        $istmt = $pdo->prepare("insert into " . $GLOBALS['PREFIX'] . "core.Users (username, password, priv_admin, entity_id, channel_id, subch_id, "
            . "user_email, firstName, lastName, customer_id, passwordDate, role_id) values (?,?,?,?,?,?,?,?,?,?,?,?)");
        $ires = $istmt->execute([$username, $password, 1, 1, $skuids, $serverid, $emailid, $ftname, $ltname, $userid, $pwdate, $roleid]);
        if ($ires) {
...
function createCoreCustomer($pdata)
{
    ...
    if ($cres) {
        ...
        $retres = ['code' => 200, 'status' => 'failed', 'msg' => 'Failed to attaching site with the user.'];
    }
    echo json_encode($retres);
}
