...
require_once $base_path . 'lib/l-util.php';
require_once $base_path . 'lib/l-dashboard.php';
require_once $base_path . 'libraries/PHPExcel.php';
require_once $base_path . 'libraries/PHPExcel/IOFactory.php';
include_once '../lib/l-setTimeZone.php';

if (!isset($_SESSION)) {
}

nhRole::dieIfnoRoles(['export']);

//Replace $routes['get'] with if else
if (url::postToText('function') === 'createFilter') { //roles: export
    createFilter();
} else if (url::postToText('function') === 'createEventFilter') { //roles: export
    createEventFilter();
} else if (url::postToText('function') === 'updateFilter') { //roles: export
    updateFilter();
} else if (url::postToText('function') === 'updateEventfilter') { //roles: export
    updateEventfilter();
} else if (url::postToText('function') === 'addQueryToInfoPortal') { //roles: export
    addQueryToInfoPortal();
}

//Replace $routes['POST'] with if else
if (url::postToText('function') === 'dataTableList') { //roles: export
    dataTableList();
} else if (url::postToText('function') === 'geteventDartList') { //roles: export
    geteventDartList();
} else if (url::postToText('function') === 'check_EventTitle') { //roles: export
    check_EventTitle();
} else if (url::postToText('function') === 'getEvent_List') { //roles: export
    getEvent_List();
} else if (url::postToText('function') === 'fetch_FilterDetails') { //roles: export
    fetch_FilterDetails();
} else if (url::postToText('function') === 'fetch_eventDetails') { //roles: export
    fetch_eventDetails();
} else if (url::postToText('function') === 'exportAsset') { //roles: export
    exportAsset();
} else if (url::postToText('function') === 'exportEvent') { //roles: export
    exportEvent();
} else if (url::postToText('function') === 'adhocInfoPortalData') { //roles: export
    adhocInfoPortalData();
}
...
function dataTableList()
{
    ...
    $auditRes = create_auditLog('Ad-hoc Queries', 'View', 'Success');
...
function createFilter()
{
    if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $auditRes = create_auditLog('Ad-hoc Queries', 'Create', 'Success', $_POST);
...
function updateFilter()
{
    ...
    if (isset($_SERVER['REQUEST_METHOD']) && $_SERVER['REQUEST_METHOD'] == 'POST') {
        ...
        $auditRes = create_auditLog('Ad-hoc Queries', 'Modification', 'Success', $_POST);
...
function deleteFilter()
{
    ...
    $auditRes = create_auditLog('Ad-hoc Queries', 'Delete', 'Success');
...
function fetchEventfilterData($dartno = '')
{
    $db = pdo_connect();
    if ($dartno == '') {
        $filtersql = $db->prepare("SELECT  filter_name FROM " . $GLOBALS['PREFIX'] . "agent.event_filters GROUP BY filter_name ORDER BY filter_name asc");
        $filtersql->execute();
    } else {
        $filtersql = $db->prepare("SELECT  filter_name FROM " . $GLOBALS['PREFIX'] . "agent.event_filters where DartNo = ? GROUP BY filter_name ORDER BY filter_name asc");
        $filtersql->execute([$dartno]);
    }
    $filterres = $filtersql->fetchAll(PDO::FETCH_ASSOC);
    return $filterres;
}
...
function fetch_FilterDetails()
{
    ...
    foreach ($filterres as $vals) {
        ...
        $assetstr1 .= "<option data-tokens=\"" . $vals['filter_name'] . "\" $selected value=\"" . $vals['filter_name'] . "\">" . $vals['filter_name'] . "</option>";
    }
    $filterarray['asset_options'] = $assetstr1;
...
function fetch_eventDetails()
{
    ...
    $filterres = fetchEventfilterData($filterarray['dartno']);
    $sourceValues = $filterarray['sourcefields'];
    ...
    foreach ($filterres as $vals) {
        ...
        $eventstr1 .= "<option data-tokens=\"" . $vals['filter_name'] . "\" $selected value=\"" . $vals['filter_name'] . "\">" . $vals['filter_name'] . "</option>";
    }
    ...
    $filterarray['asset_options'] = $eventstr1;
    $filterarray['filter_options'] = $html;
...
function createAdhocSchema($pdo)
{
    $stmt = $pdo->prepare("CREATE TABLE IF NOT EXISTS " . $GLOBALS['PREFIX'] . "agent.adhocInfoPortal (
                    id INT(10) NOT NULL AUTO_INCREMENT,
                    qid INT(10) NULL DEFAULT NULL,
                    searchtype VARCHAR(50) NULL DEFAULT NULL,
                    searchvalue VARCHAR(100) NULL DEFAULT NULL,
                    adhoctype ENUM('asset','event') NULL DEFAULT NULL,
                    queryname VARCHAR(255) NULL DEFAULT NULL,
                    datafrom INT(10) NULL DEFAULT NULL,
                    datato INT(10) NULL DEFAULT NULL,
                    status CHAR(50) NULL DEFAULT NULL,
                    global INT(4) NOT NULL DEFAULT '0',
                    cronstatus INT(4) NOT NULL DEFAULT '0',
                    filename VARCHAR(200) NULL DEFAULT NULL,
                    userid INT(4) NULL DEFAULT NULL,
                    username VARCHAR(50) NULL DEFAULT NULL,
                    createdtime INT(10) NULL DEFAULT NULL,
                    starttime INT(10) NULL DEFAULT NULL,
                    endtime INT(10) NULL DEFAULT NULL,
                    retryunit INT(10) NOT NULL DEFAULT '0',
                    PRIMARY KEY (id))
...
function addQueryToInfoPortal()
{

    $pdo = pdo_connect();

    $qtype = url::postToText('type');
    $qid = url::postToText('qid');
    $from = (url::postToAny('from') != '') ? strip_tags(strtotime(url::postToAny('from'))) : 0;
    $to = (url::postToAny('to')) ? strip_tags(strtotime(url::postToAny('to'))) : 0;

    $searchtype = $_SESSION['searchType'];
    $searchvalue = $_SESSION['searchValue'];
    $userid = $_SESSION['user']['userid'];
    $username = ($_SESSION["user"]["logged_username"] != '') ? $_SESSION["user"]["logged_username"] : 'admin';

    if ($qtype == 'asset') {
        $table = 'adhoc_query_filters';
    } else {
        $table = 'adhoc_event_filters';
    }

    $aq_stmt = $pdo->prepare("select name from " . $GLOBALS['PREFIX'] . "agent.$table where id = ?;");
    $aq_stmt->execute([$qid]);
    $qa_data = $aq_stmt->fetch(PDO::FETCH_ASSOC);
    $queryname = $qa_data['name'];

    createAdhocSchema($pdo);

    $stmt = $pdo->prepare('insert into ' . $GLOBALS['PREFIX'] . 'agent.adhocInfoPortal (qid, searchtype, searchvalue, adhoctype, datafrom, datato, '
        . 'queryname, status, global, cronstatus, userid, username, createdtime) values (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)');
    $res = $stmt->execute([$qid, $searchtype, $searchvalue, $qtype, $from, $to, $queryname, 'Pending', 1, 0, $userid, $username, time()]);
...
function adhocInfoPortalData()
{
    ...
    $stmt = $pdo->prepare("select * from " . $GLOBALS['PREFIX'] . "agent.adhocInfoPortal where searchvalue = ?");
    ...
    foreach ($data as $value) {
        ...
        if ($filename != '') {
            $fileetodownload = "<a href='" . $value['filename'] . "'>" . $filename . "</a>";
        } else {
            $fileetodownload = 'NA';
        }
        ...
        $return[] = [
            ...
            'scope' => $value['searchvalue'],
            'status' => $value['status'],
            'time' => date("m/d/Y h:i A", $value['createdtime']),
            'downloadfile' => $fileetodownload
        ];
    }

    if (safe_count($return) == 0) {
        $return = [];
    }

    $returndata = array('data' => $return);
