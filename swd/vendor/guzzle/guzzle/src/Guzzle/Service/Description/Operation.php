...
class Operation implements OperationInterface
{
    ...
    public function toArray()
    {
        ...
        foreach (safe_array_keys(self::$properties) as $check) {
            if ($value = $this->{$check}) {
                $result[$check] = $value;
            }
        }
    ...
    {
        return safe_array_keys($this->parameters);
    }
    ...
    public function setResponseType($responseType)
    {
        ...
        if (!isset($types[$responseType])) {
            throw new InvalidArgumentException('responseType must be one of ' . implode(', ', safe_array_keys($types)));
        }
