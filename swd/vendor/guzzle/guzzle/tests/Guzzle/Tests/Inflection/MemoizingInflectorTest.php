...
class MemoizingInflectorTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testProtectsAgainstCacheOverflow()
    {
        ...
        $this->assertEquals(10, safe_count($cache['snake']));
        $this->assertEquals(10, safe_count($cache['camel']));
        ...
        $cache = $this->readAttribute($inflector, 'cache');
        $this->assertEquals(9, safe_count($cache['snake']));
        $this->assertEquals(9, safe_count($cache['camel']));
    }
}
