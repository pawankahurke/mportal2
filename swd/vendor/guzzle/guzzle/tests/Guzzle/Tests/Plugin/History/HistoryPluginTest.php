...
class HistoryPluginTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testAddsRequests()
    {
        ...
        $this->assertEquals(1, safe_count($h));
        $i = $h->getIterator();
        $this->assertEquals(1, safe_count($i));
    ...
    public function testMaintainsLimit()
    {
        ...
        $this->assertEquals(2, safe_count($h));
        $i = 0;
    ...
    public function testClearsHistory()
    {
        $h = new HistoryPlugin();
        $requests = $this->addRequests($h, 5);
        $this->assertEquals(5, safe_count($h));
        $h->clear();
        $this->assertEquals(0, safe_count($h));
    }
    ...
    public function testCanCastToString()
    {
        ...
        $request = $client->get();
        $request->send();
        $this->assertEquals(3, safe_count($h));
        $this->assertEquals(3, safe_count($mock->getReceivedRequests()));
