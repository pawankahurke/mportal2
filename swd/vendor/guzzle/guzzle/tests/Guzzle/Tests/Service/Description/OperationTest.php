...
class OperationTest extends \Guzzle\Tests\GuzzleTestCase
{
    ...
    public function testContainsApiErrorInformation()
    {
        ...
        $this->assertEquals(1, safe_count($command->getErrorResponses()));
        $arr = $command->toArray();
        $this->assertEquals(1, safe_count($arr['errorResponses']));
        $command->addErrorResponse(400, 'Foo', 'Baz\\Bar');
        $this->assertEquals(2, safe_count($command->getErrorResponses()));
        $command->setErrorResponses(array());
        $this->assertEquals(0, safe_count($command->getErrorResponses()));
    }
