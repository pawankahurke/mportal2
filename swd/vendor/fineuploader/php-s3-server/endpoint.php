...
function getRequestMethod() {
    ...
    if (url::requestToAny('_method') != null) {
        return url::requestToAny('_method');
    }
...
function deleteObject() {
    getS3Client()->deleteObject(array(
        'Bucket' => url::requestToAny('bucket'),
        'Key' => url::requestToAny('key')
    ));
}
...
function signRestRequest($headersStr) {
    $version = url::issetInRequest('v4') ? 4 : 2;
...
function isPolicyValid($policy) {
    ...
    for ($i = 0; $i < safe_count($conditions); ++$i) {
...
function shouldIncludeThumbnail() {
    $filename = url::requestToAny('name');
    $isPreviewCapable = url::requestToBoolean('isBrowserPreviewCapable') === true;
