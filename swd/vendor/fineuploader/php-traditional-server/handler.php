...
class UploadHandler {
    ...
    public function getName() {
        if (url::issetInRequest('qqfilename'))
            return url::requestToAny('qqfilename');
    ...
    public function combineChunks($uploadDirectory, $name = null) {
        $uuid = url::postToAny('qquuid');
        if ($name === null) {
            $name = $this->getName();
        }
        $targetFolder = $this->chunksFolder . DIRECTORY_SEPARATOR;
        $totalParts = url::issetInRequest('qqtotalparts') ? (int) url::requestToAny('qqtotalparts') : 1;
    ...
    public function handleUpload($uploadDirectory, $name = null) {
        ...
        $size = $file['size'];
        if (url::issetInRequest('qqtotalfilesize')) {
            $size = url::requestToAny('qqtotalfilesize');
        }
        ...
        $uuid = url::requestToAny('qquuid');
        if ($totalParts > 1) {
            # chunked upload

            $chunksFolder = $this->chunksFolder;
            $partIndex = (int) url::requestToAny('qqpartindex');
    ...
    public function handleDelete($uploadDirectory, $name = null) {
        ...
        $tokens = explode('/', $url);
        $uuid = $tokens[safe_sizeof($tokens) - 1];
