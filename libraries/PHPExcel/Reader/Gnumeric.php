		...
		foreach($gnmXML->Sheets->Sheet as $sheet) {
			...
			if ((!$this->_readDataOnly) && (isset($sheet->PrintInformation))) {
				if (isset($sheet->PrintInformation->Margins)) {
					foreach($sheet->PrintInformation->Margins->children('gnm',TRUE) as $key => $margin) {
						...
						$marginSize = 72 / 100;							switch($marginAttributes['PrefUnit']) {
							case 'mm' :
			...
			foreach($sheet->Cells->Cell as $cell) {
				...
				if ($ExprID > '') {
					...
					switch($ValueType) {
						case '10' :									$type = PHPExcel_Cell_DataType::TYPE_NULL;
							break;
						case '20' :									$type = PHPExcel_Cell_DataType::TYPE_BOOL;
							$cell = ($cell == 'TRUE') ? True : False;
							break;
						case '30' :									$cell = intval($cell);
						case '40' :									$type = PHPExcel_Cell_DataType::TYPE_NUMERIC;
							break;
						case '50' :									$type = PHPExcel_Cell_DataType::TYPE_ERROR;
							break;
						case '60' :									$type = PHPExcel_Cell_DataType::TYPE_STRING;
							break;
						case '70' :								case '80' :							}
				}
