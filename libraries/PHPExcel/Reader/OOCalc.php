							...
							foreach($rowData as $key => $cellData) {
								...
								if ($hasCalculatedValue) {
									...
									foreach($temp as &$value) {
																				if ($tKey = !$tKey) {
											$value = preg_replace('/\[([^\.]+)\.([^\.]+):\.([^\.]+)\]/Ui','$1!$2:$3',$value);    											$value = preg_replace('/\[([^\.]+)\.([^\.]+)\]/Ui','$1!$2',$value);       											$value = preg_replace('/\[\.([^\.]+):\.([^\.]+)\]/Ui','$1:$2',$value);    											$value = preg_replace('/\[\.([^\.]+)\]/Ui','$1',$value);                  											$value = PHPExcel_Calculation::_translateSeparator(';',',',$value,$inBraces);
										}
									}
									unset($value);
