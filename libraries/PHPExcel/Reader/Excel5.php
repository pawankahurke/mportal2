						...
						foreach ($ranges as $range) {
							...
							if (safe_count($explodes) == 2) {
								if ($docSheet = $this->_phpExcel->getSheetByName($explodes[0])) {
									...
									if (safe_count($coordinateStrings) == 2) {
	...
	private function _readNote()
	{
		...
		if ($this->_version == self::XLS_BIFF8) {
			...
			if ($cellAddress == '$B$65536') {
				...
				$extension = true;
				$cellAddress = array_pop(safe_array_keys($this->_phpSheet->getComments()));
			}
	...
	private function _readXf()
	{
		...
		if (!$this->_readDataOnly) {
			...
			if ($isCellStyleXf) {
				...
				$this->_mapCellXfIndex[$this->_xfIndex] = safe_count($this->_phpExcel->getCellXfCollection()) - 1;
			}
	...
	private function _readLabelSst()
	{
		...
		if (($this->getReadFilter() !== NULL) && $this->getReadFilter()->readCell($columnString, $row + 1, $this->_phpSheet->getTitle())) {
			...
			if (($fmtRuns = $this->_sst[$index]['fmtRuns']) && !$this->_readDataOnly) {
				...
				$charPos = 0;
				$sstCount = safe_count($this->_sst[$index]['fmtRuns']);
