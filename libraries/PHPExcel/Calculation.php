...
class PHPExcel_Calculation
{
	...
	private static $_PHPExcelFunctions = array(
		...
		'$DE'				=> array(
			'category'			=>	PHPExcel_Calculation_Function::CATEGORY_FINANCIAL,
			'functionCall'		=>	'PHPExcel_Calculation_Financial::$DE',
			'argumentCount'	=>	'2'
		),
		'$FR'				=> array(
			'category'			=>	PHPExcel_Calculation_Function::CATEGORY_FINANCIAL,
			'functionCall'		=>	'PHPExcel_Calculation_Financial::$FR',
			'argumentCount'	=>	'2'
		),
		'DPRODUCT'				=> array(
			'category'			=>	PHPExcel_Calculation_Function::CATEGORY_DATABASE,
	...
	public function _translateFormulaToLocale($formula)
	{
		if (self::$functionReplaceFromExcel === NULL) {
			...
			foreach (safe_array_keys(self::$_localeFunctions) as $excelFunctionName) {
				self::$functionReplaceFromExcel[] = '/(@?[^\w\.])' . preg_quote($excelFunctionName) . '([\s]*\()/Ui';
			}
			foreach (safe_array_keys(self::$_localeBoolean) as $excelBoolean) {
	...
	public function calculateCellValue(PHPExcel_Cell $pCell = NULL, $resetLog = TRUE)
	{
		...
		if ((is_array($result)) && (self::$returnArrayAsType != self::RETURN_ARRAY_AS_ARRAY)) {
			...
			if (safe_count($testResult) != 1) {
				$r = safe_array_keys($result);
				$r = array_shift($r);
				if (!is_numeric($r)) {
					return PHPExcel_Calculation_Functions::VALUE();
				}
				if (is_array($result[$r])) {
					$c = safe_array_keys($result[$r]);
					$c = array_shift($c);
					if (!is_numeric($c)) {
						return PHPExcel_Calculation_Functions::VALUE();
					}
	...
	{
		$matrixRows = safe_count($matrix);
		$matrixColumns = 0;
		foreach ($matrix as $rowKey => $rowValue) {
			$matrixColumns = max(safe_count($rowValue), $matrixColumns);
	...
	private static $_operatorPrecedence	= array(
		':' => 8,																		'|' => 7,																		'~' => 6,																		'%' => 5,																		'^' => 4,																		'*' => 3, '/' => 3, 															'+' => 2, '-' => 2,																'&' => 1,																		'>' => 0, '<' => 0, '=' => 0, '>=' => 0, '<=' => 0, '<>' => 0
	);
	...
	private static function _dataTestReference(&$operandData)
	{
		...
		if (($operandData['reference'] === NULL) && (is_array($operand))) {
			$rKeys = safe_array_keys($operand);
			$rowKey = array_shift($rKeys);
			$cKeys = safe_array_keys(safe_array_keys($operand[$rowKey]));
			...
			if (isset(self::$_binaryOperators[$token])) {
				...
				switch ($token) {
					...
					case '|':
						...
						foreach (safe_array_keys($rowIntersect) as $row) {
							$oRow[] = $row;
	...
	{
		if (is_array($operand)) {
			if ((safe_count($operand, COUNT_RECURSIVE) - safe_count($operand)) == 1) {
				do {
	...
	{
		return safe_array_keys(self::$_PHPExcelFunctions);
	}
