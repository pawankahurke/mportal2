...
class PHPExcel_Calculation_DateTime {
	...
	public static function TIME($hour = 0, $minute = 0, $second = 0) {
		...
		if ($hour > 23) {
			...
			case PHPExcel_Calculation_Functions::RETURNDATE_PHP_NUMERIC :
					return (integer) PHPExcel_Shared_Date::ExcelToPHP(PHPExcel_Shared_Date::FormattedPHPToExcel(1970, 1, 1, $hour, $minute, $second));				case PHPExcel_Calculation_Functions::RETURNDATE_PHP_OBJECT :
					$dayAdjust = 0;
					if ($hour < 0) {
	...
	public static function DATEVALUE($dateValue = 1) {
		...
		if ((safe_count($t1) == 1) && (strpos($t,':') != false)) {
						return 0.0;
		} elseif (safe_count($t1) == 2) {
						if ($yearFound) {
				array_unshift($t1,1);
			} else {
