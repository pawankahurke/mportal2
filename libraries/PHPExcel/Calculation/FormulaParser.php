...
class PHPExcel_Calculation_FormulaParser {
    ...
    public function getTokenCount() {
    	return safe_count($this->_tokens);
    }
		...
		while ($index < $formulaLength) {
				...
				if (strlen($value) > 1) {
					if (preg_match("/^[1-9]{1}(\.[0-9]+)?E{1}$/", $this->_formula{$index}) != 0) {
						...
						if ($this->_formula{$index} == PHPExcel_Calculation_FormulaParser::QUOTE_DOUBLE) {
				if (strlen($value > 0)) {  					$tokens1[] = new PHPExcel_Calculation_FormulaToken($value, PHPExcel_Calculation_FormulaToken::TOKEN_TYPE_UNKNOWN);
					$value = "";
				}
				$inString = true;
				++$index;
