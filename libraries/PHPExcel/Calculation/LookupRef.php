...
class PHPExcel_Calculation_LookupRef
{
	...
	public static function CHOOSE()
	{
		...
		$entryCount = safe_count($chooseArgs) - 1;
	...
	public static function MATCH($lookup_value, $lookup_array, $match_type = 1)
	{
		...
		$lookupArraySize = safe_count($lookup_array);
		...
		if ($match_type == 1) {
			asort($lookup_array);
			$keySet = safe_array_keys($lookup_array);
		} elseif ($match_type == -1) {
			arsort($lookup_array);
			$keySet = safe_array_keys($lookup_array);
		}
	...
	public static function INDEX($arrayValues, $rowNum = 0, $columnNum = 0)
	{
		...
		$rowKeys = safe_array_keys($arrayValues);
		$columnKeys = @safe_array_keys($arrayValues[$rowKeys[0]]);

		if ($columnNum > safe_count($columnKeys)) {
			return PHPExcel_Calculation_Functions::VALUE();
		} elseif ($columnNum == 0) {
			if ($rowNum == 0) {
				return $arrayValues;
	...
	public static function VLOOKUP($lookup_value, $lookup_array, $index_number, $not_exact_match = true)
	{
		...
		if ((!is_array($lookup_array)) || (empty($lookup_array))) {
			...
			$f = safe_array_keys($lookup_array);
			$firstRow = array_pop($f);
			if ($index_number > safe_count($lookup_array[$firstRow])) {
				return PHPExcel_Calculation_Functions::REF();
			} else {
				$columnKeys = safe_array_keys($lookup_array[$firstRow]);
	...
	public static function LOOKUP($lookup_value, $lookup_vector, $result_vector = null)
	{
		...
		$lookupRows = safe_count($lookup_vector);
		$l = safe_array_keys($lookup_vector);
		$l = array_shift($l);
		$lookupColumns = safe_count($lookup_vector[$l]);
		if ((($lookupRows == 1) && ($lookupColumns > 1)) || (($lookupRows == 2) && ($lookupColumns != 2))) {
			$lookup_vector = self::TRANSPOSE($lookup_vector);
			$lookupRows = safe_count($lookup_vector);
			$l = safe_array_keys($lookup_vector);
			$lookupColumns = safe_count($lookup_vector[array_shift($l)]);
		}

		if (is_null($result_vector)) {
			$result_vector = $lookup_vector;
		}
		$resultRows = safe_count($result_vector);
		$l = safe_array_keys($result_vector);
		$l = array_shift($l);
		$resultColumns = safe_count($result_vector[$l]);
		if ((($resultRows == 1) && ($resultColumns > 1)) || (($resultRows == 2) && ($resultColumns != 2))) {
			$result_vector = self::TRANSPOSE($result_vector);
			$resultRows = safe_count($result_vector);
			$r = safe_array_keys($result_vector);
			$resultColumns = safe_count($result_vector[array_shift($r)]);
		}
