	...
	private function _readPropertySets() {
		...
		while ($offset < $entryLen) {
			...
			$this->props[] = array (
				...
				$this->wrkbook = safe_count($this->props) - 1;
			}
			else if ( $upName === 'ROOT ENTRY' || $upName === 'R') {
								$this->rootentry = safe_count($this->props) - 1;
			}

						if ($name == chr(5) . 'SummaryInformation') {
				$this->summaryInformation = safe_count($this->props) - 1;
			}

						if ($name == chr(5) . 'DocumentSummaryInformation') {
				$this->documentSummaryInformation = safe_count($this->props) - 1;
			}
