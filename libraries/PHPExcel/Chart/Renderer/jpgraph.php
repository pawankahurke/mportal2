...
class PHPExcel_Chart_Renderer_jpgraph
{
	...
	{
		$plotMarkKeys = safe_array_keys(self::$_markSet);
		if (is_null($markerID)) {
	...
	private function _renderPlotLine($groupID, $filled = false, $combination = false, $dimensions = '2d')
	{
		...
		$labelCount = safe_count($this->_chart->getPlotArea()->getPlotGroupByIndex($groupID)->getPlotValuesByIndex(0)->getPointCount());
		if ($labelCount > 0) {
	...
	private function _renderPlotScatter($groupID, $bubble)
	{
		...
		for ($i = 0; $i < $seriesCount; ++$i) {
			...
			if ($scatterStyle == 'lineMarker') {
				...
				list($splineDataY, $splineDataX) = $spline->Get(safe_count($dataValuesX) * self::$_width / 20);
	...
	private function _renderPlotStock($groupID)
	{
		...
		for ($j = 0; $j < safe_count($dataValues[0]); $j++) {
			for ($i = 0; $i < $seriesCount; $i++) {
	...
	private function _renderPieChart($groupCount, $dimensions = '2d', $doughnut = False, $multiplePlots = False)
	{
		...
		for ($groupID = 0; $groupID < $iLimit; ++$groupID) {
			...
			for ($j = 0; $j < $jLimit; ++$j) {
				...
				$seriesPlot->SetColor(self::$_colourSet[self::$_plotColour++]);
				if (safe_count($datasetLabels) > 0)
					$seriesPlot->SetLabels(array_fill(0, safe_count($datasetLabels), ''));
				if ($dimensions != '3d') {
	...
	public function render($outputDestination)
	{
		...
		if ($groupCount == 1) {
			...
			if (safe_count($chartTypes) == 1) {
				$chartType = array_pop($chartTypes);
			} elseif (safe_count($chartTypes) == 0) {
