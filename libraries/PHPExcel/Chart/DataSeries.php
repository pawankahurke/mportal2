...
class PHPExcel_Chart_DataSeries
{
	...
	public function __construct($plotType = null, $plotGrouping = null, $plotOrder = array(), $plotLabel = array(), $plotCategory = array(), $plotValues = array(), $smoothLine = null, $plotStyle = null)
	{
		...
		$keys = safe_array_keys($plotValues);
		$this->_plotValues = $plotValues;
		if ((safe_count($plotLabel) == 0) || (is_null($plotLabel[$keys[0]]))) {
			$plotLabel[$keys[0]] = new PHPExcel_Chart_DataSeriesValues();
		}

		$this->_plotLabel = $plotLabel;
		if ((safe_count($plotCategory) == 0) || (is_null($plotCategory[$keys[0]]))) {
	...
	{
		$keys = safe_array_keys($this->_plotLabel);
	...
	{
		$keys = safe_array_keys($this->_plotCategory);
		if (in_array($index, $keys)) {
	...
	{
		$keys = safe_array_keys($this->_plotValues);
		if (in_array($index, $keys)) {
	...
	{
		return safe_count($this->_plotValues);
	}
