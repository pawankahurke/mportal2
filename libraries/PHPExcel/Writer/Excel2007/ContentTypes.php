...
class PHPExcel_Writer_Excel2007_ContentTypes extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	public function writeContentTypes(PHPExcel $pPHPExcel = null, $includeCharts = FALSE)
	{
		...
		for ($i = 0; $i < $sheetCount; ++$i) {
			if (safe_count($pPHPExcel->getSheet($i)->getComments()) > 0) {
				$this->_writeOverrideContentType(
					$objWriter,
					'/xl/comments' . ($i + 1) . '.xml',
		...
		if ($pPHPExcel->hasRibbonBinObjects()) {
			$tabRibbonTypes = array_diff($pPHPExcel->getRibbonBinObjects('types'), safe_array_keys($aMediaContentTypes));
		...
		$sheetCount = $pPHPExcel->getSheetCount();
		for ($i = 0; $i < $sheetCount; ++$i) {
			if (safe_count($pPHPExcel->getSheet()->getHeaderFooter()->getImages()) > 0) {
