...
class PHPExcel_Writer_Excel2007_Worksheet extends PHPExcel_Writer_Excel2007_WriterPart
{
	...
	private function _writeSheetPr(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
	{
				...
				if($pSheet->getParent()->hasMacros()){			if($pSheet->hasCodeName()==false){
	...
	{
				if (safe_count($pSheet->getColumnDimensions()) > 0)  {
	...
	private function _writeDataValidations(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
	{
			...
			$objWriter->writeAttribute('count', safe_count($dataValidationCollection));
	...
	{
		if (safe_count($pSheet->getProtectedCells()) > 0) {
	...
	{
		if (safe_count($pSheet->getMergeCells()) > 0) {
	...
	private function _writeAutoFilter(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null)
	{
		...
		if (!empty($autoFilterRange)) {
			...
			if (safe_count($columns > 0)) {
				foreach($columns as $columnID => $column) {
					$rules = $column->getRules();
					if (safe_count($rules > 0)) {
	...
	private function _writeCell(PHPExcel_Shared_XMLWriter $objWriter = null, PHPExcel_Worksheet $pSheet = null, $pCellAddress = null, $pStringTable = null, $pFlippedStringTable = null)
	{
		if (is_array($pStringTable) && is_array($pFlippedStringTable)) {
			...
			if (is_object($cellValue) || $cellValue !== '') {
					...
					case 'inlinestr':						case 's':								case 'b':									$objWriter->writeAttribute('t', $mappedType);
						break;
					case 'f':									$calculatedValue = ($this->getParentWriter()->getPreCalculateFormulas()) ?
						...
						break;
					case 'e':									$objWriter->writeAttribute('t', $mappedType);
				}

								switch (strtolower($mappedType)) {
					case 'inlinestr':							if (! $cellValue instanceof PHPExcel_RichText) {
						...
						break;
					case 's':									if (! $cellValue instanceof PHPExcel_RichText) {
						...
						break;
					case 'f':									$attributes = $pCell->getFormulaAttributes();
						...
						break;
					case 'n':															$objWriter->writeElement('v', str_replace(',', '.', $cellValue));
						break;
					case 'b':									$objWriter->writeElement('v', ($cellValue ? '1' : '0'));
						break;
					case 'e':									if (substr($cellValue, 0, 1) == '=') {
							$objWriter->writeElement('f', substr($cellValue, 1));
	...
	{
				if (safe_count($pSheet->getComments()) > 0) {
	...
	{
				if (safe_count($pSheet->getHeaderFooter()->getImages()) > 0) {
