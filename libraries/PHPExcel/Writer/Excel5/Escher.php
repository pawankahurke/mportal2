	...
	public function close()
	{
		...
		case 'PHPExcel_Shared_Escher_DggContainer':
			...
			$recVerInstance |= $recInstance << 4;
						...
						$dggData =
				pack('VVVV'
					, $this->_object->getSpIdMax() 					, $this->_object->getCDgSaved() + 1 					, $this->_object->getCSpSaved()
		...
		case 'PHPExcel_Shared_Escher_DggContainer_BstoreContainer':
				...
				foreach ($BSECollection as $BSE) {
					...
					$innerData .= $writer->close();
						...
						$recVer			= 0xF;
			$recInstance	= safe_count($this->_object->getBSECollection());
			$recType		= 0xF001;
			$length			= strlen($innerData);
		...
		case 'PHPExcel_Shared_Escher_DgContainer':
			...
			$header = pack('vvV', $recVerInstance, $recType, $length);
						...
						$countShapes = safe_count($this->_object->getSpgrContainer()->getChildren());
		...
		case 'PHPExcel_Shared_Escher_DgContainer_SpgrContainer_SpContainer':
			...
			$this->_spTypes[] = ($this->_object->getSpType());
						...
						$recVer			= 0x2;
			$recInstance	= $this->_object->getSpType(); 			$recType		= 0xF00A;
			$length			= 0x00000008;
			...
			$data .= $header . pack('VV', $this->_object->getSpId(), $this->_object->getSpgr() ? 0x0005 : 0x0A00);
				...
				$recVer			= 0x3;
				$recInstance	= safe_count($this->_object->getOPTCollection());
				$recType		= 0xF00B;
