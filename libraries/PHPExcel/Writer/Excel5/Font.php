...
class PHPExcel_Writer_Excel5_Font
{
	...
	public function writeFont()
	{
		...
		$icv = $this->_colorIndex; 		if ($this->_font->getSuperScript()) {
			$sss = 1;
		...
		$bFamily = 0; 		$bCharSet = PHPExcel_Shared_Font::getCharsetFromFontName($this->_font->getName()); 
		$record = 0x31;				$reserved = 0x00;			$grbit = 0x00;				if ($this->_font->getItalic()) {
			$grbit |= 0x02;
		}
		...
		$data = pack("vvvvvCCCC",
			$this->_font->getSize() * 20,									$grbit,
			$icv,															self::_mapBold($this->_font->getBold()),						$sss,															self::_mapUnderline($this->_font->getUnderline()),
			$bFamily,
			$bCharSet,
			$reserved
		);
