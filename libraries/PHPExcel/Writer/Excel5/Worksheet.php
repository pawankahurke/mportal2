...
class PHPExcel_Writer_Excel5_Worksheet extends PHPExcel_Writer_Excel5_BIFFwriter
{
								...
								$parser, $preCalculateFormulas, $phpSheet)
	{
		...
		$this->_countCellStyleXfs = safe_count($phpSheet->getParent()->getCellStyleXfCollection());
	}
	...
	function close()
	{
		...
		for ($i = 0; $i <= $maxCol; ++$i) {
			...
			$colcount = safe_count($this->_colinfo);
		...
		if (!empty($autoFilterRange)) {
			...
			$xfIndex = $rowDimension->getXfIndex() + 15; 			$this->_writeRow( $rowDimension->getRowIndex() - 1, $rowDimension->getRowHeight(), $xfIndex, ($rowDimension->getVisible() ? '0' : '1'), $rowDimension->getOutlineLevel() );
		}
	...
	private function _writeBIFF8CellRangeAddressFixed($range = 'A1')
	{
		$explodes = explode(':', $range);
				...
				if (safe_count($explodes) == 1) {
			$lastCell = $firstCell;
		} else {
			$lastCell = $explodes[1];
		}

		$firstCellCoordinates = PHPExcel_Cell::coordinateFromString($firstCell); 		$lastCellCoordinates  = PHPExcel_Cell::coordinateFromString($lastCell);
		return(pack('vvvv',
	...
	private function _writeRichTextString($row, $col, $str, $xfIndex, $arrcRun){
		$record	= 0x00FD;				   		$length	= 0x000A;				   		$str = PHPExcel_Shared_String::UTF8toBIFF8UnicodeShort($str, $arrcRun);
	...
	{
		$strlen	= strlen($str);
		$record	= 0x0204;				   		$length	= 0x0008 + $strlen;
		$str_error = 0;

		if ($strlen > $this->_xls_strmax) { 			$str	= substr($str, 0, $this->_xls_strmax);
	...
	{
		$record = 0x0207;	 		$data = PHPExcel_Shared_String::UTF8toBIFF8UnicodeLong($stringValue);
	...
	private function _writeWindow2()
	{
		...
		$grbit		  = 0x00B6;	 		$rwTop		  = 0x0000;	 		$colLeft		= 0x0000;

				$fDspFmla	   = 0;					 		$fDspGrid	   = $this->_phpSheet->getShowGridlines() ? 1 : 0; 		$fDspRwCol	  = $this->_phpSheet->getShowRowColHeaders() ? 1 : 0; 		$fFrozen		= $this->_phpSheet->getFreezePane() ? 1 : 0;				$fDspZeros	  = 1;					 		$fDefaultHdr	= 1;					 		$fArabic		= $this->_phpSheet->getRightToLeft() ? 1 : 0; 		$fDspGuts	   = $this->_outline_on;			$fFrozenNoSplit = 0;					 				$fSelected	  = ($this->_phpSheet === $this->_phpSheet->getParent()->getActiveSheet()) ? 1 : 0;
		$fPaged		 = 1;					 		$fPageBreakPreview = $this->_phpSheet->getSheetView()->getView() === PHPExcel_Worksheet_SheetView::SHEETVIEW_PAGE_BREAK_PREVIEW;
		...
		$data	= pack("vvv", $grbit, $rwTop, $colLeft);
				...
				$rgbHdr	   = 0x0040; 		$zoom_factor_page_break = ($fPageBreakPreview? $this->_phpSheet->getSheetView()->getZoomScale() : 0x0000);
	...
	private function _writeSelection()
	{
		...
		$colFirst = PHPExcel_Cell::columnIndexFromString($colFirst) - 1; 		--$rwFirst;
		list($colLast, $rwLast) = PHPExcel_Cell::coordinateFromString($last);
		$colLast = PHPExcel_Cell::columnIndexFromString($colLast) - 1; 		--$rwLast;
		...
		$pnn	  = $this->_active_pane;	 		$rwAct	= $rwFirst;						$colAct   = $colFirst;			   		$irefAct  = 0;					   		$cref	 = 1;
		if (!isset($rwLast)) {
			$rwLast   = $rwFirst;	   		}
		if (!isset($colLast)) {
			$colLast  = $colFirst;	  		}
	...
	private function _writeMergedCells()
	{
		...
		$countMergeCells = safe_count($mergeCells);
	...
	private function _writeSheetLayout()
	{
		...
		$recordData = pack(
			'vvVVVvv'
			, 0x0862
			, 0x0000					, 0x00000000				, 0x00000000				, 0x00000014				, $this->_colors[$this->_phpSheet->getTabColor()->getRGB()]				, 0x0000				);
	...
	private function _writeExternsheet($sheetname)
	{
		$record	= 0x0017;
			...
			$sheetname = '';
			$length	= 0x02;  			$cch	   = 1;	 			$rgch	  = 0x02;  		} else {
	...
	private function _writePanes()
	{
		...
		if (safe_count($panes) > 4) { 			$pnnAct = $panes[4];
		} else {
			$pnnAct = null;
		}
	...
	private function _writeSetup()
	{
		...
		$iPageStart   = 0x01;				 		$iFitWidth	= (int) $this->_phpSheet->getPageSetup()->getFitToWidth();			$iFitHeight	= (int) $this->_phpSheet->getPageSetup()->getFitToHeight();			$grbit		= 0x00;				 		$iRes		 = 0x0258;			   		$iVRes		= 0x0258;
		$numHdr	   = $this->_phpSheet->getPageMargins()->getHeader();
		$numFtr	   = $this->_phpSheet->getPageMargins()->getFooter();   		$iCopies	  = 0x01;
		$fLeftToRight = 0x0;
	...
	private function _writeWsbool()
	{
		$record	  = 0x0081;   		$length	  = 0x0002;   		$grbit	   = 0x0000;
										...
										$grbit |= 0x0001;						   		if ($this->_outline_style) {
			$grbit |= 0x0020; 		}
	...
	private function _writeBreaks()
	{
		...
		foreach ($this->_phpSheet->getBreaks() as $cell => $breakType) {
			...
			$record  = 0x001b;			   			$cbrk	= safe_count($hbreaks);	   			$length  = 2 + 6 * $cbrk;
			$header  = pack("vv", $record, $length);
			$data	= pack("v",  $cbrk);
			...
			$record  = 0x001a;			   			$cbrk	= safe_count($vbreaks);	   			$length  = 2 + 6 * $cbrk;
			$header  = pack("vv",  $record, $length);
			$data	= pack("v",   $cbrk);
	...
	function _positionImage($col_start, $row_start, $x1, $y1, $width, $height)
	{
		...
		$x2 = $width  / PHPExcel_Shared_Excel5::sizeCol($this->_phpSheet, PHPExcel_Cell::stringFromColumnIndex($col_end))	 * 1024; 		$y2 = $height / PHPExcel_Shared_Excel5::sizeRow($this->_phpSheet, $row_end + 1)	 *  256;
	...
	function _processBitmap($bitmap)
	{
		...
		$data = substr($data, 4);
		if ($planes_and_bitcount[2] != 24) { 			throw new PHPExcel_Writer_Exception("$bitmap isn't a 24bit true color bitmap.\n");
		}
	...
	private function _writeMsoDrawing()
	{
			...
			$spTypes = $writer->getSpTypes();
						...
						$spOffsets[0] = 0;
			$nm = safe_count($spOffsets) - 1; 			for ($i = 1; $i <= $nm; ++$i) {
								$record = 0x00EC;
	...
	private function _writeDataValidity()
	{
			...
			$data		= pack('vVVVV', $grbit, $horPos, $verPos, $objId,
										 safe_count($dataValidationCollection));
			$this->_append($header.$data);
	...
	private function _writePageLayoutView(){
		...
		$rt         = 0x088B; 		$grbitFrt   = 0x0000; 		$reserved   = 0x0000000000000000; 		$wScalvePLV = $this->_phpSheet->getSheetView()->getZoomScale();
	...
	private function _writeCFRule(PHPExcel_Style_Conditional $conditional){
		...
		$numConditions = safe_sizeof($arrConditions);
		if($numConditions == 1){
		...
		$flags |= (1 == 1              ? 0x00004000 : 0); 		$flags |= (1 == 1              ? 0x00008000 : 0); 				$flags |= (1 == $bFillStyle    ? 0x00010000 : 0);
		...
		if($bFormatFill == 1){
			...
			switch ($conditional->getStyle()->getFill()->getFillType()){
				...
				case PHPExcel_Style_Fill::FILL_GRADIENT_LINEAR			: $blockFillPatternStyle = 0x00; break;					case PHPExcel_Style_Fill::FILL_GRADIENT_PATH			: $blockFillPatternStyle = 0x00; break;					default                                                 : $blockFillPatternStyle = 0x00; break;
			}
	...
	private function _writeCFHeader(){
		...
		$header	  = pack('vv', $record, $length);
		$data	  = pack('vv', safe_count($arrConditional), $needRedraw);
		$data     .= $cellRange;
		$data     .= pack('v', 0x0001);
		$data     .= $cellRange;
		$this->_append($header . $data);
