...
class PHPExcel_Writer_Excel5 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	private function _buildWorksheetEschers()
	{
		...
		foreach ($this->_phpExcel->getAllsheets() as $sheet) {
			...
			if (safe_count($sheet->getDrawingCollection()) == 0 && empty($filterRange)) {
				continue;
			}
	...
	private function _buildWorkbookEscher()
	{
		$escher = null;
				...
				$found = false;
		foreach ($this->_phpExcel->getAllSheets() as $sheet) {
			if (safe_count($sheet->getDrawingCollection()) > 0) {
				$found = true;
				break;
			}
		}
		...
		$dggContainer->setBstoreContainer($bstoreContainer);
			...
			foreach ($sheet->getDrawingCollection() as $drawing) {
				if ($drawing instanceof PHPExcel_Worksheet_Drawing) {
					...
					case 1: 						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
					...
					case 2: 						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_JPEG;
						$blipData = file_get_contents($filename);
						break;

					case 3: 						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						$blipData = file_get_contents($filename);
						break;

					case 6: 						$blipType = PHPExcel_Shared_Escher_DggContainer_BstoreContainer_BSE::BLIPTYPE_PNG;
						ob_start();
	...
	private function _writeDocumentSummaryInformation(){
		...
		$dataSection_Content = '';
				...
				$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x01),
							   'offset' => array('pack' => 'V'),
							   'type' 	=> array('pack' => 'V', 'data' => 0x02), 							   'data'	=> array('data' => 1252));
		...
		foreach ($dataSection as $dataProp){
			...
			elseif($dataProp['type']['data'] == 0x0B){ 				if($dataProp['data']['data'] == false){
	...
	private function _writeSummaryInformation(){
		...
		$dataSection_NumProps++;
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x02),
								   'offset' => array('pack' => 'V'),
								   'type' 	=> array('pack' => 'V', 'data' => 0x1E), 								   'data'	=> array('data' => $dataProp, 'length' => strlen($dataProp)));
			$dataSection_NumProps++;
		}
			...
			$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x0C),
								   'offset' => array('pack' => 'V'),
								   'type' 	=> array('pack' => 'V', 'data' => 0x40), 								   'data'	=> array('data' => PHPExcel_Shared_OLE::LocalDate2OLE($dataProp)));
			$dataSection_NumProps++;
		}
			...
			$dataSection_NumProps++;
				...
				$dataSection[] = array('summary'=> array('pack' => 'V', 'data' => 0x13),
							   'offset' => array('pack' => 'V'),
							   'type' 	=> array('pack' => 'V', 'data' => 0x03), 							   'data'	=> array('data' => 0x00));
		$dataSection_NumProps++;
