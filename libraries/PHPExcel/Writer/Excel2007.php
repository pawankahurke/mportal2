...
class PHPExcel_Writer_Excel2007 extends PHPExcel_Writer_Abstract implements PHPExcel_Writer_IWriter
{
	...
	public function __construct(PHPExcel $pPHPExcel = null)
	{
		...
		$hashTablesArray = array(
			'_stylesConditionalHashTable',	'_fillHashTable',		'_fontHashTable',
	...
	public function save($pFilename = null)
	{
		if ($this->_spreadSheet !== NULL) {
			...
			for ($i = 0; $i < $this->_spreadSheet->getSheetCount(); ++$i) {
				...
				$drawingCount = safe_count($drawings);
				if ($this->_includeCharts) {
				...
				if (safe_count($this->_spreadSheet->getSheet($i)->getComments()) > 0) {
				...
				if (safe_count($this->_spreadSheet->getSheet($i)->getHeaderFooter()->getImages()) > 0) {
