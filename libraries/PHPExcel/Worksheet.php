...
class PHPExcel_Worksheet implements PHPExcel_IComparable
{
    ...
    {
        return safe_count($this->_chartCollection);
    }


    public function getChartByIndex($index = null)
    {
        $chartCount = safe_count($this->_chartCollection);
        if ($chartCount == 0) {
            return false;
        }
        if (is_null($index)) {
	...
	public function setCodeName($pValue=null){
		...
		$pValue = str_replace(' ', '_', $pValue);		        		self::_checkSheetCodeName($pValue);
