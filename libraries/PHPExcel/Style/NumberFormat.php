...
class PHPExcel_Style_NumberFormat extends PHPExcel_Style_Supervisor implements PHPExcel_IComparable
{
	...
	private static function _formatAsDate(&$value, &$format)
	{
		...
		if (!strpos($format,'A')) {				$format = strtr($format,self::$_dateFormatReplacements24);
		} else {								$format = strtr($format,self::$_dateFormatReplacements12);
		}
	...
	public static function toFormattedString($value = '0', $format = PHPExcel_Style_NumberFormat::FORMAT_GENERAL, $callBack = null)
	{
		...
		$format = preg_replace($color_regex, '', $format);
				...
				if (preg_match('/^(\[\$[A-Z]*-[0-9A-F]*\])*[hmsdy]/i', $format)) { 			self::_formatAsDate($value, $format);
		} else if (preg_match('/%$/', $format)) { 			self::_formatAsPercentage($value, $format);
		} else {
